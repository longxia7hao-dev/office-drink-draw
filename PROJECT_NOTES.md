...
- Grok 於 gh-pages 部署 `5a4235a` 後，公開站真機出現「載入失敗」；已在 `codex/mobile-overhaul` 合併最新 gh-pages 後，恢復已驗證的穩定手機 hotfix，準備以 fast-forward 推回 gh-pages。
- 目前待部署版本：e7dcce1979ad0338；功能基底提交：17e6a5d；本次恢復提交待補。
- 本機重新驗證：30 項手機回歸通過、0 失敗；edge 檢查通過（Chromium/WebKit 初始工作室圖 6 張、700ms 內換幀、tap 後音訊請求 2、閒置音訊 0）。
